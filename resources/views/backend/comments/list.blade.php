...
@section('title','Hiển thị bình luận')
@section('bread')
        ...
        <li>
            <div class="page-header">
                <nav class="breadcrumb-one" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{url('admin/binh-luan')}}">Bình luận</a></li>
...
@section('content')
    <table id="style-3" class="table style-3  table-hover">
        ...
        <tr>
            ...
            <th class="text-center"> ProductImage </th>
        ...
        <tbody id="load">
        @foreach($comments as $c => $item)
            @if($item->parent_id == 0)
            <tr>
                <td class="checkbox-column text-center"> {{$loop->iteration}} </td>
                <td class="text-center">{{$item->product->name}}</td>
                <td class="text-center">
                    <span><img src="{{asset('/storage/products/'. $item->product->image)}}" class="profile-img" alt="image"></span>
                </td>
                    <td class="text-center">{{$item->user->name}}</td>
                    <td class="text-center">
                        <span><img src="{{$item->user->image}}" class="profile-img" alt="image"></span>
                    </td>
                    <td class="text-center">{{$item->user->role == 100 ? "Quản trị viên":"Thành viên"}}</td>
                <td class="text-center">{{$item->comment}}</td>
                <td class="text-center"><a href="{{url('admin/binh-luan/reply/'.$item->id)}}">Reply</a></td>
                @if($item->status == 1)
